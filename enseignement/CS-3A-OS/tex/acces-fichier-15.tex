...
\begin{tikzpicture}[ultra thick]
    ...
    \node[draw,fill=red!40,minimum width=1.8cm,minimum height=1cm] (dir2) at (-10,2.8) {\small Répertoire};
    \node[draw,fill=red!40,minimum width=1.5cm,minimum height=.8cm] (fcb2) at (-6,1.6) {\small FCB};

    \draw[-Stealth, ultra thick] (fcb2) to (.8,2);
    

    \node at (2,9.5) {\begin{minipage}{6cm}
				\textbf{Accès au fichier ouvert\ldots}
				\end{minipage}};
	\node at (-9,7.5) {\texttt{\small read(dir/file.ext)}};

    \draw[dashed,-Stealth, ultra thick] (-9.5,7) to (fcb2);
    \node[draw,fill=red!40,minimum width=1cm,minimum height=1cm] at (1.5,2) {\small Bloc};

    \node[draw,fill=red!40,minimum width=1cm,minimum height=1cm] (file1b) at (-6,7.5) {\small Bloc};
			\draw[thick,dashed,-Stealth, ultra thick] (file1) to (file1b);
			\node at (-2,5.5) {\textbf{accès disque}};
